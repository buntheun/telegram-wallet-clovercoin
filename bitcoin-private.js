...
db.on('connect', function () {
    console.log('Haciendo de la base de datos de trabajo....')
})
...
module.exports.newAccount = function newAccount(uniqueID, callback){
        ...
        db.accounts.find({UniqueId:uniqueID}, function (err, res) {
            ...
            if(!err && res == ""){
                ...
                return callback({error:true, response:"Usted ya tiene una cuenta, intente el /address comando o /ayuda"});
            }
        });
...
module.exports.getAccountAddress = function getAccountAddress(uniqueID, callback){
        ...
        db.accounts.find({UniqueId:uniqueID}, function (err, res) {
            if(!err && res != ""){
                ...
                return callback({error:true, response:"¿No tienes una cuenta todavia? trate de /newAccount o /help"});
            }
        })
}
...
module.exports.checkBalance  = function checkBalance(address, callback){
    // Set the headers
    var address = config.config.blockExploreUrl + "/ext/getbalance/" + address + "";
    console.log(address);
    var headers = {
